...
$app->group('/v1', function (\Slim\Routing\RouteCollectorProxy $group) use ($container) {

    // Define routes
    $group->get('/healthcheck', function (Request $request, Response $response) {
        $payload = json_encode(['app' => true]);
        $response->getBody()->write($payload);
        return $response->withHeader('Content-Type', 'application/json');
    });

    $group->group('/flights', function (\Slim\Routing\RouteCollectorProxy $group) use ($container) {
    ...
    $group->group('/passengers', function (\Slim\Routing\RouteCollectorProxy $group) {
